...
"""
Unit tests for MLU support in cuda_kernel_launch_stats.

These tests verify that the `cuda_kernel_launch_stats` method in
`CudaKernelAnalysis` correctly handles MLU runtime events
(mluLaunchKernel, mluLaunchKernelExC, mluMemcpyAsync, mluMemsetAsync)
...
class TestMluKernelLaunchStats(unittest.TestCase):
    """Tests for MLU support in CudaKernelAnalysis.cuda_kernel_launch_stats."""

    def test_mlu_launch_kernel_events_are_captured(self):
        """Verify mluLaunchKernel events are included in the launch stats."""
        # Symbol IDs
        sym_index = {
            "mluLaunchKernel": 100,
        }
    ...
    def test_mlu_launch_kernel_ex_c_events_are_captured(self):
        """Verify mluLaunchKernelExC events are included in the launch stats."""
        sym_index = {
            "mluLaunchKernelExC": 101,
        }
    ...
    def test_mlu_memory_events_included(self):
        """Verify mluMemcpyAsync and mluMemsetAsync are included when include_memory_events=True."""
        sym_index = {
            "mluMemcpyAsync": 102,
            "mluMemsetAsync": 103,
        }
    ...
    def test_mlu_memory_events_excluded(self):
        """Verify mluMemcpyAsync/mluMemsetAsync are excluded when include_memory_events=False."""
        sym_index = {
            "mluLaunchKernel": 100,
            "mluMemcpyAsync": 102,
            "mluMemsetAsync": 103,
        ...
        # Only mluLaunchKernel should be captured, not the memory events
        self.assertEqual(len(df), 1)
        self.assertEqual(df.iloc[0]["correlation"], 1001)

    def test_mixed_cuda_and_mlu_events(self):
        """Verify both CUDA and MLU events are captured together in a mixed trace."""
        sym_index = {
            "cudaLaunchKernel": 10,
            "mluLaunchKernel": 100,
            "cudaMemcpyAsync": 11,
    ...
    def test_negative_launch_delay_clipped_to_zero(self):
        """Verify that negative launch delays are clipped to 0 for MLU events."""
        sym_index = {
            "mluLaunchKernel": 100,
        }
    ...
    def test_mlu_only_no_cuda_symbols(self):
        """Verify the function works when only MLU symbols exist (pure ROCm trace)."""
        sym_index = {
            "mluLaunchKernel": 100,
            "mluLaunchKernelExC": 101,
            "mluMemcpyAsync": 102,
    ...
    def test_empty_trace_returns_empty_result(self):
        ...
        # Trace with no events matching mluLaunchKernel
        trace_df = pd.DataFrame(
            {
                "name": [999, 998],
                "stream": [-1, 1],
    ...
    def test_multiple_ranks_with_mlu_events(self):
        """Verify MLU events are processed correctly for multiple ranks."""
        sym_index = {
            "mluLaunchKernel": 100,
        }
